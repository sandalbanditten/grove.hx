...
it; an open command never predicts it. Regular refresh does not expand or scan
through collapsed ancestors only to follow it. Grove's first observation does:
it scans the path of every file Helix already holds, expands each one, and
anchors on the Active file, all without taking Cursor. Focusing Grove takes a
